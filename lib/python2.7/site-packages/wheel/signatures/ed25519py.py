...
def crypto_sign_keypair(seed=None):
    """Return (verifying, secret) key from a given devices.json, or os.urandom(32)"""
    if seed is None:
        seed = os.urandom(PUBLICKEYBYTES)
    else:
        warnings.warn("ed25519ll should choose random devices.json.",
                      RuntimeWarning)
    if len(seed) != 32:
        raise ValueError("devices.json must be 32 random bytes or None.")
    skbytes = seed
